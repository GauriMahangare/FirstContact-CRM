...
class TeamListView(LoginRequiredMixin, SuccessMessageMixin, ListView):
    ...
    def get_context_data(self, **kwargs: Any):
        team_qs = Team.objects.filter(
            organisation=self.request.user.userorganization
...
class TeamUpdateView(LoginRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    waffle_flag = "update_team"
    slug_field = "slug"  # The name of the field on the model that contains the slug
    slug_url_kwarg = (
        "slug_text"  # The name of the URLConf keyword argument that contains the slug
    )
    ...
    def form_valid(self, form):
        if waffle.flag_is_active(self.request, "update_team"):
...
class TeamDetailView(
    LoginRequiredMixin, SuccessMessageMixin, WaffleFlagMixin, DetailView
):
    model = Team
    template_name = "teams/team_detail.html"
    context_object_name = "team"
    waffle_flag = "detail_team"
    slug_field = "slug"  # The name of the field on the model that contains the slug
    slug_url_kwarg = (
        "slug_text"  # The name of the URLConf keyword argument that contains the slug
    )
    ...
    def form_valid(self, form):
        if waffle.flag_is_active(self.request, "create_team"):
            ...
            if not team.name:
                ...
                messages.success(
                    self.request,
                    "Congratulations!! Team has been set; Now assign team members to this team",
                )
                return redirect("/teams/~redirect/")
        else:
            messages.error(
...
class TeamMembersView(
    ...
    def get_context_data(self, **kwargs: Any):
        ...
        if qs.exists():
            ...
            return HttpResponse("<h1> Page not found. </h1>")
        team_qs = TeamMembership.objects.filter(team=team.pk).order_by(
            "-dateTimeModified"
        )
        context = super().get_context_data(**kwargs)
        paginator = Paginator(team_qs, self.paginate_by)
...
def add_member_view(request, *args, **kwargs):
    ...
    context = {"form": form, "user_defined": user_defined, "team_defined": team_defined}
