        ...
        if waffle.flag_is_active(self.request, "create_lead"):
            ...
            if not lead.email:
                ...
                # Update user data so that org cannot be created any more by this user
                    ...
                    messages.success(
                        self.request,
                        "Congratulations!! lead has been added; Now assign a members to this lead",
                    )
...
def multi_lead_delete_view(request, *args, **kwargs):
    ...
    for lead in leads_tobe_deleted_list:
        try:
            ...
            print(str(e))
            logger.critical(
                "GET:There was an error deleting your lead  %s", lead, exc_info=1
            )
            raise Http404(
            ...
            if request.method == "POST":
                try:
                    ...
                    print(str(e))
                    logger.critical(
                        "POST:There was an error deleting your lead  %s",
                        lead,
                        exc_info=1,
                    )
                    raise Http404(
    ...
    def form_valid(self, form):
        if waffle.flag_is_active(self.request, "create_note"):
            ...
            try:
                ...
                return redirect(
                    reverse("leads:listnote", args=[leadid]) + "?" + urlparameters
                )
            else:
        ...
        if waffle.flag_is_active(self.request, "create_category"):
            ...
            if not category.status:
                ...
                try:
                    ...
                    return redirect(reverse("leads:listcategory"))
        else:
            messages.error(
                self.request,
                "Please update your subscription to continue using this feature.",
