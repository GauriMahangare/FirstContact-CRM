...
def lead_profile_image_directory_path(instance, filename):
    ...
    return "org_{0}/leads/{1}/profile_picture/{2}".format(
...
class Lead(models.Model):
    ...
    created_by = models.ForeignKey(
        User, null=True, on_delete=models.SET_NULL, related_name="lead_added_by"
    )
    ...
    work_address_country = models.ForeignKey(
    ...
    emailOptIn = models.BooleanField(
        "Email Opt In", blank=True, null=True, default=False
    )
...
def lead_note_attachment_directory_path(instance, filename):
    ...
    print(instance.id)
    print(filename)
    return "org_{0}/leads/{1}/note_attachments/{2}/{3}".format(
        org_name_no_space, instance.lead.id, instance.id, filename
    )


class Note(models.Model):
    created_by = models.ForeignKey(
        User, null=True, on_delete=models.SET_NULL, related_name="note_added_by"
    )
    ...
    file = models.FileField(
        null=True, blank=True, upload_to=lead_note_attachment_directory_path
    )
