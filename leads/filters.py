...
def category_of_same_org(request):
    if request is None:
        print("request is empty")
        return Category.objects.none()

    org = request.user.userorganization
    return Category.objects.filter(organisation=org)
...
class LeadFilter(django_filters.FilterSet):
    ...
    annual_revenue__gt = django_filters.NumberFilter(
        field_name="annual_revenue", lookup_expr="gt"
    )
    annual_revenue__lt = django_filters.NumberFilter(
        field_name="annual_revenue", lookup_expr="lt"
    )

    nbr_of_employees = django_filters.NumberFilter()
    nbr_of_employees__gt = django_filters.NumberFilter(
        field_name="nbr_of_employees", lookup_expr="gt"
    )
    nbr_of_employees__lt = django_filters.NumberFilter(
        field_name="nbr_of_employees", lookup_expr="lt"
    )

    status = django_filters.ModelMultipleChoiceFilter(
        queryset=category_of_same_org,
        widget=CheckboxSelectMultiple(
            attrs={
                "class": "form-control",
            }
