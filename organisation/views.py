    ...
    def form_valid(self, form):
        if waffle.flag_is_active(self.request, "create_organisation"):
            ...
            if not organisation.work_org_name:
                ...
                messages.success(
                    self.request,
                    "Congratulations!! Organisation set up is complete; Now invite your team members to collaborate.. Happy working!",
                )
...
class OrganisationUserListView(LoginRequiredMixin, SuccessMessageMixin, ListView):
    ...
    def get_context_data(self, **kwargs: Any):
        user_qs = Invitation.objects.filter(inviter_id=self.request.user.pk).order_by(
            "-sent"
        )
        # for u in user_qs:
        #     print(u.email)
...
class OrganisationUserReinviteView(LoginRequiredMixin, SuccessMessageMixin, View):
    def post(self, request, pk, *args, **kwargs):
        try:
            ...
            print(str(e))
            logger.info(
                "Error deleting user invitation:User invitation does not exist."
            )
            messages.error(
                request, "The user you are trying to invite has been already removed."
            )
        ...
        if not user.accepted:
            ...
            try:
                ...
                messages.error(
                    request,
                    "There was an error in reinviting the user. Please try after sometime",
                )
...
class OrganisationUserdeleteView(LoginRequiredMixin, SuccessMessageMixin, View):
    def get(self, request, pk, *args, **kwargs):
        try:
            ...
            messages.error(
                request, "The user you are trying to delete has been removed already."
            )
            return redirect("/organisation/~userlist/")
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        try:
            ...
            messages.error(
                request,
                "There was an error deleting the user. Please try after sometime",
            )
        ...
        if accepted:
            try:
                ...
                messages.error(
                    request,
                    "The user you are trying to delete has been removed already.",
                )
                return redirect("/organisation/~userlist/")
            try:
                user.delete()
